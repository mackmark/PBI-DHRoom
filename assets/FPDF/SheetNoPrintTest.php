...
$query1 = "SELECT Top 1 sd.DateCreated as 'prodate2', s.SheetNo, s.DateCreated as 'prodDate', cb.CurringBooth ";
$query1 .= "FROM sheetdetail_tbl sd ";
$query1 .= "LEFT JOIN sheet_tbl s ON sd.SheetID = s.SheetID ";
$query1 .= "LEFT JOIN curringbooth_tbl cb ON sd.CurringBoothID = cb.CurringBoothID ";
$query1 .= "WHERE s.SheetID = '".$cat_id."' order by sd.DateCreated ASC ";
$resultQuery1 = odbc_exec($conn, $query1);
$count = odbc_num_rows($resultQuery1);
$prodate2 = "";
$sheetNo = "";
$cb = "";
while($rowQuery1 = odbc_fetch_array($resultQuery1)){
    $prodate2 = $rowQuery1['prodate2'];
...
$pdf->Cell(20,5,date('d-M-y', strtotime($prodate2)),0,0, 'L');
$pdf->Cell(10,5,'',0,0, 'C');
$pdf->SetFont('Helvetica','',8);
